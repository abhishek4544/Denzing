    ...
    >
      ...
      >
        ...
        <Section title="Layers">
          {Array.from({ length: settings.layerCount }).flatMap((_, i) => {
            const isLast = i === settings.layerCount - 1;
            const nodes = [
              <ColorField
                key={`color-${i}`}
                label={`Layer ${i + 1}`}
                color={settings.layerColors[i] ?? settings.meshColor}
                opacity={100}
                onColorChange={(v) => setLayerColor(i, v)}
                onOpacityChange={() => {}}
                showPipette={false}
              />,
              <SelectField
                ...
                onChange={(v) => setLayerContentType(i, v as ContentType)}
